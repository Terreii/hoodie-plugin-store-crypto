...
test('cryptoStore.resetPassword() should work with pouchdb-hoodie-api', async t => {
  t.plan(11)

  const { db, cryptoStore } = createPouchCryptoStore()

  try {
    const resetKeys = await cryptoStore.setup('test')
    ...
    const resetKey = getRandomItemOfArray(resetKeys)
    const report = await cryptoStore.resetPassword(resetKey, 'nextPassword')
